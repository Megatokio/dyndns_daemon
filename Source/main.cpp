...
static bool ping_self_ok(cstr url)
{
	...
	else { logline("%s: %s", url, curl_easy_strerror(res)); }

	curl_easy_cleanup(curl);
	return ok;
}

__attribute__((unused)) static inline cstr ip4addrstr(uint32 ip)
{
	...
	for (;;)
	{
		...
		for (;;)
		{
			...
			if (*s == '.' && dots < 3)
			{
				...
				s++;
				continue;
			}	   // more dot-separated numbers to got
			break; // format error
		}
...
__attribute__((noreturn)) static void dyndns_updater()
{
	...
	for (;;)
	{
		...
		try
		{
			...
			else { logline("webserver not running"); }
		}
...
static void parse_config(cstr configfile)
{
	...
	for (;;)
	{
		...
		if (eq(key, "verbose"))
		{
			verbose = !strchr("0nN", *val);
			continue;
		}
		if (eq(key, "daemon"))
		{
			foreground = strchr("0nN", *val);
			continue;
		}
		if (eq(key, "ip4"))
		{
			update_ip4 = !strchr("0nN", *val);
			continue;
		}
		if (eq(key, "ip6"))
		{
			update_ip6 = !strchr("0nN", *val);
			continue;
		}
...
int main(int argc, cstr argv[])
{
	try
	{
		...
		{
			printf(
				"dyndns_updater (c) 2015-2021 kio@little-bat.de\n"
	...
	{
		abort("%s: unexpected error: %s", APPL_NAME, e);
	}
	catch (AnyError& e)
	{
		abort("%s: unexpected error: %s", APPL_NAME, e.what());
	}
	catch (std::exception& e)
	{
		abort("%s: unexpected exception: %s", APPL_NAME, e.what());
	}
}
